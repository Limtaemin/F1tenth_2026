...
<div>
  ...
  onmousedown="send('left_down')"
  onmouseup="send('left_up')"
  ontouchstart="send('left_down')"
  ontouchend="send('left_up')">L</button>
  <button class="dpad-btn center-btn" onclick="send('stop')">X</button>
  <button class="dpad-btn"
  onmousedown="send('right_down')"
  onmouseup="send('right_up')"
  ontouchstart="send('right_down')"
  ontouchend="send('right_up')">R</button>
</div>
<div>
...
def cmd():
    ...
    with lock:
        # --- speed stays incremental ---
        if c == "up":
            speed += MAX_SPEED
        elif c == "down":
            speed -= MAX_SPEED

        # --- steering is press-and-hold ---
        elif c == "left_down":
            steer = +STEER_MAG
        elif c == "right_down":
            steer = -STEER_MAG
        elif c in ("left_up", "right_up"):
            steer = 0.0
